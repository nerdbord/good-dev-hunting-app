...
function MessageElementWithTranslation({
    ...
    const label = element.translationKey && t
 ...
 */
export function formatMessageElement(content: string, elementName: string): string {
...
export function parseMessageContent(content: string): React.ReactNode {
    ...
    for (const element of messageElements) {
        ...
        const matches = [...remainingContent.matchAll(new RegExp(element.pattern, 'g'))];
        ...
        if (matches.length > 0) {
            ...
            for (const match of matches) {
                ...
                if (match.index > lastIndex) {
                    ...
                    if (beforeText) {
                        segments.push(<React.Fragment key={`segment-${segmentIndex++}`}>{beforeText}</React.Fragment>);
                    }
                }
                ...
                segments.push(
                    <React.Fragment key={`element-${element.name}-${segmentIndex++}`}>
                        <MessageElementWithTranslation element={element} content={elementContent} />
                    </React.Fragment>
                );
