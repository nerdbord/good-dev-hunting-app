 ...
 */
function hasJobDataChanged(
  existingJob: JobWithRelations,
  newData: JobDataWithTechStack,
): boolean {
  ...
  if (
    newData.employmentTypes &&
    !safeCompare(newData.employmentTypes, existingJob.employmentTypes)
  ) {
    return true
  }

  if (
    newData.employmentModes &&
    !safeCompare(newData.employmentModes, existingJob.employmentModes)
  ) {
    return true
  }
  ...
  if (newData.techStack?.connectOrCreate && existingJob.techStack) {
    console.log(`[hasJobDataChanged] Comparing tech stack:`, {
      ...
      newTechStack: newData.techStack.connectOrCreate.map(
        (item: { create: { name: string } }) => item.create.name,
      ),
    })

    const existingTechNames = existingJob.techStack
      .map((tech) => tech.name)
      .sort()
    const newTechNames = newData.techStack.connectOrCreate
      .map((item: { create: { name: string } }) => item.create.name)
      .sort()
...
export const updateJobAction = withSentry(
  async (id: string, data: JobDataWithTechStack) => {
    ...
    const updatedJob = await updateJob(id, {
      ...
      ...(hasChanged && job.state !== PublishingState.DRAFT
