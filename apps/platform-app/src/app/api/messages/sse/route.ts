    ...
    const stream = new ReadableStream({
      start(controller) {
        ...
        if (!global.messageListeners) {
          global.messageListeners = new Map<
            string,
            Set<typeof messageHandler>
          >()
        }

        // Set up a listener for messages to this negotiation
        if (!global.messageListeners.has(applicationId)) {
          global.messageListeners.set(
            applicationId,
            new Set<typeof messageHandler>(),
          )
        }
