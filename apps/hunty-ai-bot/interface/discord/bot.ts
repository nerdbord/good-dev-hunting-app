import { ChatInputCommandInteraction, Client, GatewayIntentBits, Message, REST, Routes, SlashCommandBuilder, type Interaction } from 'discord.js';
...
export class DiscordBot {
    ...
    private async registerCommands() {
        ...
        try {
            ...
            if (Bun.env.DISCORD_GUILD_ID) {
                ...
                await rest.put(
                    Routes.applicationGuildCommands(this.client.user.id, Bun.env.DISCORD_GUILD_ID),
                    { body: [] }
                );
    ...
    private async handleRecordCommand(interaction: ChatInputCommandInteraction) {
        ...
        await interaction.editReply('Started recording! Use `/stop` to end recording.');
    }
