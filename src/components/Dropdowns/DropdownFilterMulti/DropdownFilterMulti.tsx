...
export const DropdownFilterMulti = ({
  ...
  options,
  jobOfferFilterName,
  hasSearchInput = false, // Default value for optional prop
  value = [], // Default value for optional prop
  onSearch,
}: DropdownFilterMultiProps) => {
  const [arrow, setArrow] = useState('IoIosArrowDown')
  const [isDropdownActive, setDropdownActive] = useState(false)
  ...
  const handleSelect = (option: string) => {
    setSelectedValue((prev) => {
      const isExists = prev.includes(option)
      return isExists ? prev.filter((p) => p !== option) : [...prev, option]
    })
  }
  ...
  useEffect(() => {
    onSearch(jobOfferFilterName, selectedValue.join(','))
  }, [selectedValue, onSearch, jobOfferFilterName])
